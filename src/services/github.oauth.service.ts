...
class GitHubOAuthService {
   ...
   public getToken(code: string, host: URL) {
      ...
      params.append(
         'client_secret',
         'ed46bd3a0f736e0da57308e86ca5fa3cf8688582',
      );
      ...
      promise.catch((err) => {
         console.error('Sync: Invalid GitHub Enterprise URL.', err);
      });
   ...
   public async getGists(token: string, user: string, host: URL) {
      const promise = fetch(
         `https://api.${host.hostname}/users/${user}/gists`,
         {
            method: 'GET',
   ...
   public async saveToken(token: string) {
      console.log('Set settings ', token);
   }
